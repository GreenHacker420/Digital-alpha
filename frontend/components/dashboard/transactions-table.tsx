...
export function TransactionsTable({
  ...
  if (error) {
    return <div className="table-state table-state--error"><strong>Transactions couldn’t load.</strong><span>Check the API connection and try again.</span></div>;
  }
  ...
  return (
    <div className="table-shell" aria-busy={fetching}>
          ...
          <tbody>
            {loading
              ...
              : data?.items.map((transaction) => (
                  ...
                  >
                    <td><div className="merchant-cell"><span className="merchant-mark">{transaction.merchant_name.slice(0, 1).toUpperCase()}</span><div><strong>{transaction.merchant_name}</strong><small>{transaction.reference_id ?? transaction.id.slice(0, 12)}</small></div></div></td>
                    ...
                    <td className="amount-cell"><strong>{formatMoney(transaction.amount, transaction.currency)}</strong></td>
                  </tr>
                ))}
          </tbody>
        </table>
      ...
      {!loading && data?.items.length === 0 ? (
        <div className="table-state"><strong>No transactions match.</strong><span>Try clearing one or more filters.</span></div>
      ) : null}

      <footer className="table-footer">
        <span>{data ? `${data.total.toLocaleString("en-IN")} transactions` : "Loading transactions…"}{fetching && !loading ? " · Updating…" : ""}</span>
        <div className="pagination">
          <Button variant="secondary" disabled={!data || data.page <= 1 || fetching} onClick={() => data && onPage(data.page - 1)}>←</Button>
          <span>{data?.pages ? `Page ${data.page} of ${data.pages}` : "—"}</span>
          <Button variant="secondary" disabled={!data || data.page >= data.pages || fetching} onClick={() => data && onPage(data.page + 1)}>→</Button>
        </div>
      </footer>
    </div>
  );
